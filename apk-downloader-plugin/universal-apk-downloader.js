...
async function findOnApkPure(appName) {
  try {
    ...
    let link =
      $('a.dd').first().attr('href') ||
      $('div.search-dl a').first().attr('href') ||
      $('a[href*="/app/"]').first().attr('href');
...
async function getApkDirectFromApkPure(appUrl) {
  try {
    ...
    const dlRel =
      $('a.da').first().attr('href') ||
      $('a[href*="/download?"]').first().attr('href');
    const dlPage = dlRel
      ? dlRel.startsWith('http')
        ? dlRel
        : `https://apkpure.com${dlRel}`
      : null;
    ...
    const match =
      html2.match(/https?:\/\/download\.apkpure\.com\/b\/[^\"]+/) ||
      html2.match(/https?:\/\/r\d+\.apkpure\.com\/[^\"]+/);

    if (match) return match[0];

    const $2 = cheerio.load(html2);
    const direct =
      $2('a#download_link').attr('href') ||
      $2('a[href*="download.apkpure.com"]').attr('href');
...
async function findOnApkCombo(appName) {
  try {
    ...
    const first =
      $('.search-list a').first().attr('href') ||
      $('a.item-title').first().attr('href');
...
async function downloadAnyApk({ queryOrUrl, filename, folder = './downloads' }) {
  ...
  if (isDirectApkUrl(queryOrUrl)) {
    ...
    if (!res.ok)
      throw new Error(`Failed to download direct APK: ${res.status}`);
  ...
  // detect if it's Play Store link
  if (appName.includes('play.google.com/store/apps/details?id=')) {
    const id = appName.split('id=')[1];
    console.log(`Redirecting Play Store link to APKPure for ID: ${id}`);
    queryOrUrl = `https://apkpure.com/store/apps/details?id=${id}`;
  }
  ...
  const sources = [
    async () => {
      ...
      return apkUrl
        ? { apkUrl, sourcePage: appUrl, sourceName: 'apkpure' }
        : null;
    },
    async () => {
      ...
      return apkUrl
        ? { apkUrl, sourcePage: appUrl, sourceName: 'apkcombo' }
        : null;
    }
  ];
  ...
  for (const attempt of sources) {
    try {
      ...
      console.log('Source failed, trying next one...', e.message);
    }
  }
  ...
  if (!r.ok)
    throw new Error(`Failed to download APK from ${finalUrl}: ${r.status}`);
  await streamToFile(r, savePath);

  return {
    source: found.sourceName || 'unknown',
